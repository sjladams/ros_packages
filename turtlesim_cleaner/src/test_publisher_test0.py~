...
def main():
	...
	GreenLed_publisher = rospy.Publisher('elisa3_robot_1/green_led',Bool)
	...
	while True:
		try:
			...
			#cmd_led = str(raw_input('Green Led: '))
			cmd_led = 'on'			
			GreenLed = Bool()

			if cmd_led=='on':
				GreenLed.data = True
				GreenLed_publisher.publish(GreenLed)
			else:
				GreenLed.data = False
				GreenLed_publisher.publish(GreenLed)
